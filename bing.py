...
# DRIVER_PATH = 'C:/Users/Patrick/Desktop/Scraping/edgedriver_win64/msedgedriver'
...
def fetch_image_urls(query: str, max_links_to_fetch: int, wd: webdriver, sleep_between_interactions: int = 3):
    ...
    :param sleep_between_interactions: Amount of time for the method to sleep (default at 3).
    ...
    elif is_bing is True:
        search_url = "https://www.bing.com/images/search?q={q}"

    # Load the page.
    wd.get(search_url.format(q=query))

    # This will make Google Chrome not physically be displayed on the screen when running.
    # wd.set_window_position(-10000, 0)
    ...
    while image_count < max_links_to_fetch:
        ...
        if is_chrome is True:
            ...
            # thumbnail_results = wd.find_elements(by=By.XPATH, value='//img[@class="mimg rms_img"]')  # "img.mimg.rms_img"
            # thumbnail_results = wd.find_elements(by=By.CLASS_NAME, value="mimg")
            thumbnail_results = wd.find_elements(by=By.XPATH, value='//div[@class="imgpt"]/img[@class="mimg rms_img"]')
        ...
        for img in thumbnail_results[results_start:number_results]:
            ...
            if is_chrome is True:
                ...
                # wd.switch_to_frame('OverlayIFrame')
                actual_images = wd.find_elements(by=By.XPATH, value='//div[@class="mainImage current"]/img[@src]')
                # actual_images = wd.find_elements(by=By.TAG_NAME, value='img')

            for actual_image in actual_images:
                if actual_image.get_attribute('src') and 'http' in actual_image.get_attribute('src'):
                    image_urls.add(actual_image.get_attribute('src'))
                    # wait.until(expected_conditions.element_to_be_clickable((By.CSS_SELECTOR, "div.close.nofocus"))).click()
...
def search_and_download(search_term: str, driver_path: str, number_images: int):
    ...
    # 'We are using the EdgeDriver within a "with" context, guaranteeing that the browser closes down ordinarily,
...
while True:
    ...
    with open('image-service.txt') as text:
        ...
        for line in lines:
            ...
            # If the word "Bing" is in the line...
            if "Bing" in line:
                is_bing = True

                # Then the variable "search_term" will equal to the term after the 7th character.
                search_term = line[7:]
                # Makes the number of images equal to the number between the 5th and 7th character.
                num_img = int(line[5:7])
                # Calls the function with the specified parameters.
                new_res = search_and_download(search_term=search_term, driver_path=DRIVER_PATH, number_images=num_img)
                # Replaces the particular line inside the text file with the url(s) of the photo(s).
                lines[index] = new_res + "\n"
